# -*- coding: utf-8 -*-
"""
Created on Wed Mar 25 13:58:15 2020
...
# main data cube 
xcube = iris.load_cube(f'../../Model_Data/u-bu807/nc/Control/windvector_10m/{Case}_x_wind_24hrs_vera_301.nc', 'x_wind')
xdata = xcube.data
ycube = iris.load_cube(f'../../Model_Data/u-bu807/nc/Control/windvector_10m/{Case}_y_wind_24hrs_vera_301.nc', 'y_wind')
ydata = ycube.data
...
mspcube = iris.load_cube(f'../../Model_Data/u-b807/nc/Control/air_pressure_at_sea_level/{Case}_air_pressure_at_sea_level_24hrs_verb_301.nc')
        ...
        for time in range(8):
            ...
            if (time) % 8 == 0:
                ...
                fig0,ax0,norm0 = foundry.plot_hoz_xsec_mass(magdata, lmdata, model_lon, model_lat, lons, lats, lmlon, lmlat, time, level,
                                                                 data_label = r'Windspeed, $ms^{-1}$', domain = 'fulldomain_obs_demonstration', 
                                                                 flight = 301, savefig = True, res = Case, unrotated = True,
                                                                 variable_name = 'Surface horizontal windspeed', time_norm = True,
                                                                 variable_in_file_name = '10m_windvector_mancscale', 
                                                                 figure_path_pdf = f'../../Figures/PDF/301/P_levels/windvector_pl/{Case}/{domain}/verstash/surface_10m',
                                                                 figure_path_png = f'../../Figures/PNG/301/P_levels/windvector_pl/{Case}/{domain}/verstash/surface_10m',
                                                                 contour_type = 'pcolor', colourmap = 'cool',sampling_rate = 'halfhourly',
                                                                 ...
                                                                 cscale_override = True, cscale_min = 0, cscale_max = 16,
                                                                 obvs_on = True, obvs_z = db_u10m, obvs_x = dbx[0], obvs_y = dby[:,0], PDF = False, verstash = True)
