...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 28 15:48:17 2020
...
for res in resolutions:
    ...
    #%% load and process core model diagnostics from vertical sliced files
        ...
        cube_coor = w_cube.coord(fileleg[3][0]).points
        surf_alt = w_cube.coord('surface_altitude').points
        #print(surf_alt)
        ...
        prec_div = 1000
        ### 
        ### sqrt(sum((y_i - x_i)**2)/n)
        ###
        rmsd_wsp = (np.nanmean((matched_wsp - db_wsp)**2))**0.5*prec_div//1/prec_div
        rmsd_w   = (np.nanmean((matched_w   - db_w  )**2))**0.5*prec_div//1/prec_div
        rmsd_theta= (np.nanmean((matched_theta-db_theta  )**2))**0.5*prec_div//1/prec_div
        rmsd_q   = (np.nanmean((matched_q   - db_q  )**2))**0.5*prec_div//1/prec_div                                                                                                                                                                                                           
        print('rmsd',rmsd_wsp, rmsd_w, rmsd_theta, rmsd_q)
        
        #%% mean and sd
        
        mean_wsp_um = np.nanmean(matched_wsp)*prec_div//1/prec_div
        var_wsp_um = np.nanvar(matched_wsp)*prec_div//1/prec_div
        mean_wsp_db = np.nanmean(db_wsp)*prec_div//1/prec_div
        var_wsp_db = np.nanvar(db_wsp)*prec_div//1/prec_div
        
        mean_w_um = np.nanmean(matched_w)*prec_div//1/prec_div
        var_w_um = np.nanvar(matched_w)*prec_div//1/prec_div
        mean_w_db = np.nanmean(db_w)*prec_div//1/prec_div
        var_w_db = np.nanvar(db_w)*prec_div//1/prec_div
        
        mean_theta_um = np.nanmean(matched_theta)*prec_div//1/prec_div
        var_theta_um = np.nanvar(matched_theta)*prec_div//1/prec_div
        mean_theta_db = np.nanmean(db_theta)*prec_div//1/prec_div
        var_theta_db = np.nanvar(db_theta)*prec_div//1/prec_div
        
        
        mean_q_um = np.nanmean(matched_q)*prec_div//1/prec_div
        var_q_um = np.nanvar(matched_q)*prec_div//1/prec_div
        mean_q_db = np.nanmean(db_q)*prec_div//1/prec_div
        var_q_db = np.nanvar(db_q)*prec_div//1/prec_div
        
        
        #%%
        rows_wsp.append([fileleg[0],rmsd_wsp, mean_wsp_um, var_wsp_um, mean_wsp_db, var_wsp_db,r'$ms^{-1}$',int(np.nanmean(db_alt)//1),dates[fileleg[1]]])
        rows_w.append([fileleg[0],rmsd_w, mean_w_um, var_w_um, mean_w_db, var_w_db,r'$ms^{-1}$',int(np.nanmean(db_alt)//1),dates[fileleg[1]]])
        rows_theta.append([fileleg[0],rmsd_theta, mean_theta_um, var_theta_um, mean_theta_db, var_theta_db,'K',int(np.nanmean(db_alt)//1),dates[fileleg[1]]])
        rows_q.append([fileleg[0],rmsd_q, mean_q_um, var_q_um, mean_q_db, var_q_db,r'$gkg^{-1}$',int(np.nanmean(db_alt)//1),dates[fileleg[1]]])
        
    #%%
    coloumn_labels = ['leg','rmsd','mean_um', 'var_um', 'mean_obs', 'var_obs','units', 'mean_alt_in_m','model_time']
    
    dtf_wsp = pd.DataFrame(rows_wsp, columns = coloumn_labels)
    dtf_w = pd.DataFrame(rows_w, columns = coloumn_labels)
    dtf_theta = pd.DataFrame(rows_theta, columns = coloumn_labels)
    dtf_q = pd.DataFrame(rows_q, columns = coloumn_labels)    
    print(dtf_wsp)
    print(dtf_w)
    print(dtf_theta)
    print(dtf_q)
    ...
    dtf_wsp.to_csv(f'{dir_path}{config}_{res}_windspeed_statistics_basic.csv')
    dtf_w.to_csv(f'{dir_path}{config}_{res}_upward_wind_velocity_statistics_basic.csv')
    dtf_theta.to_csv(f'{dir_path}{config}_{res}_air_potential_temperature_statistics_basic.csv')
    dtf_q.to_csv(f'{dir_path}{config}_{res}_specific_humidity_statistics_basic.csv')
