...
if surf_wind_plot:
    ...
    msp_kwargs = {'transform':data_crs, 'cmap':'viridis','vmin':mmin, 'vmax':mmax}
    ...
    ax5.pcolormesh(msp_lon, msp_lat, msp_data[0]/100, **msp_kwargs)
    ax5.contour(*lbm_args, **lbm_kwargs)
    
    ax6 = fig.add_subplot(gs[1,1], **splt_kwargs)
    ax6.pcolormesh(msp_lon, msp_lat, msp_data[1]/100, **msp_kwargs)
    ax6.contour(*lbm_args, **lbm_kwargs)
    
    ax7 = fig.add_subplot(gs[1,2], **splt_kwargs)
    ax7.pcolormesh(msp_lon, msp_lat, msp_data[2]/100, **msp_kwargs)
    ax7.contour(*lbm_args, **lbm_kwargs)
    
    ax8 = fig.add_subplot(gs[1,3], **splt_kwargs)
    ax8.pcolormesh(msp_lon, msp_lat, msp_data[3]/100, **msp_kwargs)
    ...
    cbar4 = plt.colorbar(prf_map,ax = [ax13,ax14,ax15,ax16],  orientation = 'vertical', fraction = 0.012, pad = 0.1)
    cbar4.ax.set(ylabel = r'$gm^{-2}s^{-1}$')
    title = f'UM GLM Surface Variables - Control - flt{flight}'
    ...
    #plt.savefig(f'D:/Project/Figures/PDF/{flight}/{suite}/glm/GLM_um_surf_wsp_msp_tem_prf_{suite[2:]}_flt{flight}.pdf')
    #plt.savefig(f'D:/Project/Figures/PNG/{flight}/{suite}/glm/GLM_um_surf_wsp_msp_tem_prf_{suite[2:]}_flt{flight}.png')
    plt.show()
