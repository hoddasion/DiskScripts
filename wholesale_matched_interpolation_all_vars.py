...
config = 'RA1M'
alt_idx = 30
xwind_name = 'm01s15i002'
ywind_name = 'm01s15i003'
stream_prefix = ''
...
if raw_all_points_sr:
    ...
    theta_cube = iris.load_cube(f'D:/Project/Model_Data/{suite}/{nc_path}RA1M_{res}_um_air_potential_temperature_24hrs_{stream_prefix}i_{flight}.nc', 'air_potential_temperature')[tstart_idx:tend_idx,:alt_idx]
    q_cube = iris.load_cube(f'D:/Project/Model_Data/{suite}/{nc_path}RA1M_{res}_um_specific_humidity_24hrs_{stream_prefix}i_{flight}.nc', 'specific_humidity')[tstart_idx:tend_idx,:alt_idx]
    ...
    wse_cube = iris.load_cube(f'D:/Project/Model_Data/{suite}/{nc_path}RA1M_{res}_um_atmosphere_downward_eastward_stress_24hrs_{stream_prefix}h_{flight}.nc', 'atmosphere_downward_eastward_stress')[tstart_idx:tend_idx,:alt_idx]
    wsn_cube = iris.load_cube(f'D:/Project/Model_Data/{suite}/{nc_path}RA1M_{res}_um_atmosphere_downward_northward_stress_24hrs_{stream_prefix}h_{flight}.nc','atmosphere_downward_northward_stress')[tstart_idx:tend_idx,:alt_idx]
    # regrid wsn cube and combine cubes for magnitude
    #wsn_cube_reg = wsn_cube.regrid(wse_cube, iris.analysis.Linear())
    temp_cube =  iris.load_cube(f'D:/Project/Model_Data/{suite}/{nc_path}RA1M_{res}_um_atmosphere_downward_eastward_stress_24hrs_{stream_prefix}h_{flight}.nc', 'atmosphere_downward_eastward_stress')[tstart_idx:tend_idx,:alt_idx]
    ...
    sh_cube = iris.load_cube(f'D:/Project/Model_Data/{suite}/{nc_path}RA1M_{res}_um_upward_heat_flux_in_air_24hrs_{stream_prefix}i_{flight}.nc', 'upward_heat_flux_in_air')[tstart_idx:tend_idx,:alt_idx]
    lh_cube = iris.load_cube(f'D:/Project/Model_Data/{suite}/{nc_path}RA1M_{res}_um_upward_latent_heat_flux_in_air_24hrs_{stream_prefix}i_{flight}.nc', 'upward_latent_heat_flux_in_air')[tstart_idx:tend_idx,:alt_idx]
    ...
    results_dataframe.to_csv('D:/Project/Model_Data/{suite}/Matched_interpolated_values_sr_{flight}.txt')
...
if raw_all_points_60s:
    ...
    results_dataframe = pd.DataFrame(data = np.transpose(all_results),columns = column_names)
    print(results_dataframe)
    results_dataframe.to_csv(f'D:/Project/Model_Data/{suite}/Matched_interpolated_values_60s_{flight}.txt')