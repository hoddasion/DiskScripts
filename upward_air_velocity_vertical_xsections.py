    ...
    for D in resolutions:
        for leg in legs:
            try:
                ...
                print('File error')
                continue
            path = f'../../Figures/PNG/301/u-bu807/Vertical/{varname}/Alt2000/leg{fileleg}/Norm{vmax}/{res}'
            for index in range(47):
                
        
                
                fig = workshop.plot_vertical_xsection(maincube, thetacube, orogcube,index, Case, cmap = 'seismic', vmin = vmin,vmax = vmax, levels = 30, alt_limit = 2000)
...
if obs_overlay:
    ...
    for D in resolutions:
        ...
        print(tcubes)
        #%%
        maincube = iris.load_cube(f'../../Model_Data/{suite}/nc/{exp}/Vertical_slices/{varname}/{config}_{res}_{varname}_flt{flight}_leg{fileleg}.nc', 'upward_air_velocity')
        ...
        for index in t_indices:
            fig = workshop.plot_vertical_xsection(maincube, thetacube, orogcube,
                                                  ...
                                                  theta_contour = True,alt_limit = 2000)
            plt.scatter(rot_db_lon, db_alt,c=db_w, s = 300, cmap = cmap, 
                        vmin = vmin,vmax = vmax, edgecolor = 'k')
            path = f'../../Figures/PNG/301/u-bu807/Vertical/{varname}/Alt2000/Obs/{res}'
            workshop.save_vert_xsection(fig, maincube,index, path, Case, fileleg = leg, obs = True, obs_detail = 'l89_vlim2t2')
            #plt.close()
            plt.show()
...
if plot_series:
   ...
   ## load quality controlled and interval-meaned observational data from Database
    ...
    db_w = np.array(database['w'][legcond]) # load and convert from g/kg to kg/kg
