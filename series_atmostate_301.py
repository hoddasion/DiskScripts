...
# -*- coding: utf-8 -*-
"""
Created on Sun Oct  4 15:03:14 2020
...
for fileleg in leg_meta:
    ...
    #%% load and process core model diagnostics from vertical sliced files
        ...
        ax4.grid()
        ## wsp
        ax0.plot(simple_coors_wsp, simple_wsp, label = 'UM wsp')
        ax0.plot(db_coor,db_wsp, label = 'obs')
        ...
        ax0.grid()
        ## w
        ax1.plot(simple_coors_w, simple_w, label = 'UM w')
        ax1.plot(db_coor,db_w, label = 'obs')
        ...
        ax1.grid()
        ## theta
        ax2.plot(simple_coors_t, simple_theta, label = 'UM theta')
        ax2.plot(db_coor, db_theta, label = 'obs')
        ...
        ax2.grid()
        ## specfific humidity
        ax3.plot(simple_coors_q, simple_q, label = 'UM q')
        ax3.plot(db_coor, db_q, label = 'obs')
        ax3.set_ylabel(q_cube.units)
        ...
        if save_fig:
            ...
            fig.savefig(f'../../Figures/PNG/301/{suite}/Series/{exp}/atmostate/{res}/{config}_{res}_atmostate_series_{exp}_{timepoint}_leg{fileleg[0]}.png')
        plt.show()