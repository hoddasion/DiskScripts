...
config = 'RA1M'
exp = 'CONTROL'; experiment = exp
suite = 'u-cc134'
alt_idx = 35
long = ''
factor_suffix = '_f2'

#%% make xsections for wind components
plot_wind_comps = False
if plot_wind_comps:
    for res in resolutions:
        for leg in [7]:
            ...
            try:
                ...
                xleft = 170; xright = 265
                tick_coors, tick_dists = workshop.dist_tick_generator(w_cube[:,:,xleft:xright], 'grid_latitude', ticknumber = 6, decprecision = 2)
                for tidx in range(1):
                    #%% format date time for file name and figure title
                    tidx = 29
                    time = w_cube.coord('time')
                    dates = time.units.num2date(time.points)
                    print(tidx, dates[tidx])
                    
                    #%% normalisations
                    xmin = -20; xmax = 20
                    u_norm = matplotlib.colors.Normalize(vmin = xmin, vmax = xmax)
                    u_map = matplotlib.cm.ScalarMappable(norm = u_norm, cmap = 'bwr')
                    
                    wmin = -5; wmax = 5
                    w_norm = matplotlib.colors.Normalize(vmin = wmin, vmax = wmax)
                    w_map = matplotlib.cm.ScalarMappable(norm = w_norm, cmap = 'bwr')
                    #%% commence plotting
                    #tidx = 24
                    ...
                    top_alt = 4000
                    theta_levs = np.arange(250,300,2)
                    print(theta_levs)
                    panel_left = 0.1; panel_right = 0.8
                    uv_levs = 20
                    ## u - x
                    ax0 = fig.add_subplot(gs[0,0])
                    ax0.set_ylim(top = top_alt)
                    ax0.set_title('horizontal windspeed in x direction - u')
                    #ax0.set_xticks([])
                    ax0.set_ylabel('m')
                    ax0.set_xlim(left = panel_left, right = panel_right)
                    pu = iplt.contourf(u_cube[tidx], cmap = 'bwr', vmin = xmin, vmax=xmax, **common_kwargs, levels = uv_levs)
                    p0t = iplt.contour(theta_cube[tidx], colors = 'k', levels = theta_levs, **common_kwargs)
                    
                    ## v - y
                    ax1 = fig.add_subplot(gs[1,0])
                    ax1.set_ylim(top = top_alt)
                    ax1.set_title('horizontal windspeed in y direction - v')
                    ax1.set_xticks([])
                    ax1.set_ylabel('m')
                    ax1.set_xlim(left = panel_left, right = panel_right)
                    pv = iplt.contourf(v_cube[tidx], cmap = 'bwr', vmin = xmin, vmax=xmax, **common_kwargs, levels = uv_levs)
                    p1t = iplt.contour(theta_cube[tidx], colors = 'k', levels = theta_levs, **common_kwargs)
                    
                    ## w - z
                    ax2 = fig.add_subplot(gs[2,0])
                    ax2.set_ylim(top = top_alt)
                    ax2.set_title('vertical velocity - v')
                    #ax1.set_xticks([])
                    ax2.set_ylabel('m')
                    ax2.set_xlim(left = panel_left, right = panel_right)
                    pw = iplt.contourf(w_cube[tidx], cmap = 'bwr', vmin = wmin, vmax=wmax, **common_kwargs)
                    p2t = iplt.contour(theta_cube[tidx], colors = 'k', levels = theta_levs, **common_kwargs)
            ...
            #%%
                ...
                print(f'Exception occured for leg {leg} res {res}')
                continue
...
if mass_plots:
    for res in resolutions:
        for leg in [1,2,3,4,6,7,8,9,11,12,13,15]:  
            ...
            #try:
                ...
                u_cube = iris.load_cube(f'D:/Project/Model_Data/{suite}/Vertical/x_wind/{config}_vertslice_{res}_x_wind_flt306_leg{leg}{long}{factor_suffix}.nc', 'x_wind')[:,:alt_idx]
                v_cube = iris.load_cube(f'D:/Project/Model_Data/{suite}/Vertical/y_wind/{config}_vertslice_{res}_y_wind_flt306_leg{leg}{long}{factor_suffix}.nc', 'y_wind')[:,:alt_idx]
                theta_cube = iris.load_cube(f'D:/Project/Model_Data/{suite}/Vertical/air_potential_temperature/{config}_vertslice_{res}_air_potential_temperature_flt306_leg{leg}{long}{factor_suffix}.nc', 'air_potential_temperature')[:,:alt_idx]
                q_cube = iris.load_cube(f'D:/Project/Model_Data/{suite}/Vertical/specific_humidity/{config}_vertslice_{res}_specific_humidity_flt306_leg{leg}{long}{factor_suffix}.nc', 'specific_humidity')[:,:alt_idx]
                w_cube = iris.load_cube(f'D:/Project/Model_Data/{suite}/Vertical/upward_air_velocity/{config}_vertslice_{res}_upward_air_velocity_flt306_leg{leg}{long}{factor_suffix}.nc','upward_air_velocity')[:,:alt_idx]
           
                #%% process u_cube and v_cube into wsp_cube by completely circumventing and ignoring stupid metadata issue
                wsp_cube = iris.load_cube(f'D:/Project/Model_Data/{suite}/Vertical/x_wind/{config}_vertslice_{res}_x_wind_flt306_leg{leg}{long}{factor_suffix}.nc', 'x_wind')[:,:alt_idx]
                ...
                for tidx in range(48):
                    ...
                    #            s = 300, edgecolor = 'k')
                    fig.suptitle(f'{config} Atmospheric state variables, {res}, leg {leg}{long}, {dates[tidx]}')
                    fig.tight_layout()
                    save_figure = True
                    if save_figure:
                        timepoint = dates[tidx].strftime('H%HM%M')
                        plt.savefig(f'D:/Project/Figures/PNG/306/{experiment}/{suite}/vertical/atmostate/{res}/leg{leg}{long}/{config}_{res}_atmostate_vert_xsection_{exp}_{timepoint}_leg{leg}{long}.png')
                    ...
                    if mid_point/(60*60) <= model_time_hsm[tidx] + 0.25 and mid_point/(60*60) > model_time_hsm[tidx] - 0.25:
                        ...
                        ax3.scatter(db_coor, db_alt, c = db_q, cmap = 'Blues', vmin = qmin, vmax = qmax,
                                    s = 300, edgecolor = 'k')
                        fig.suptitle(f'{config} Atmospheric state variables, {res}, leg {leg}, {dates[tidx]}')
                        fig.tight_layout()
                        save_figure = True
                        if save_figure:
                            timepoint = dates[tidx].strftime('H%HM%M')
                            plt.savefig(f'D:/Project/Figures/PNG/306/{experiment}/{suite}/vertical/atmostate/obs/{config}_{res}_atmostate_vert_xsection_obs_{exp}_{timepoint}_leg{leg}{long}.png')
                            #plt.savefig(f'D:/Project/Figures/PDF/306/{experiment}/{suite}/vertical/atmostate/obs/{config}_{res}_atmostate_vert_xsection_obs_{exp}_{timepoint}_leg{leg}{long}.pdf')
