...
figure_path_pdf = '../Figures/PDF/301/Model_levels/Theta/'
figure_path_png = '../Figures/PNG/301/Model_levels/Theta/'
figure_path_mp4 = '../Figures/MP4/301/Model_levels/Theta/'
...
file_name = '1p5km_air_potential_temperature_1.nc'
path1 = f'../Model_Data/u-bk574/nc/air_potential_temperature/'
...
lons, lats = modf.unrotate_coords(theta_lon, theta_lat, 
...
if domain_on == True:
    for level in range(40):
        for time in range(48):
            counter += 1; print(counter, 'model-level:', level + 1, 'time-step:', time + 1)
            fig = modf.make_horizontal_model_contourf_linear(theta, land_mask[0][0], theta_lon, theta_lat, lons, lats, theta_lon, theta_lat,
                                                             data_label = r'$\theta$, [K]', domain = 'fulldomain', 
                                                             flight = 301, savefig = True, res = '1p5km',
                                                             variable_name = 'Potentintal temperature', level = level, time = time,
...
# subset 4d data
sub_data_region1 = modf.subset_4d(theta, South, North, West, East)
...
sub_lon1 = modf.subset_1d(theta_lon, West, East)
sub_lat1 = modf.subset_1d(theta_lat, South, North)
...
if region1_on == True:
    for level in range(40):
        for time in range(48):
            ...
            fig = modf.make_horizontal_model_contourf_linear(sub_data_region1, sub_land_region1, sub_lon1, sub_lat1, sub_lons1, sub_lats1, sub_lon1, sub_lat1,
                                                                     data_label = r'$\theta$, [K]', domain = 'subset1', 
                                                                     flight = 301, savefig = True, res = '1p5km',
                                                                     variable_name = 'Potential temperature', level = level, time = time,
                                                                     variable_in_file_name = 'air_potential_temperature',
                                                                     figure_path_pdf = '../Figures/PDF/301/Model_levels/Theta',
                                                                     figure_path_png = '../Figures/PNG/301/Model_levels/Theta',
                                                                     contour_type = 'pcolor')
...
# subset 4d data
sub_data_region2 = modf.subset_4d(theta, South, North, West, East)
       
# two-dimensional subsetting of land-mask
sub_land_region2 = modf.subset_2d(land_mask[0][0], South, North, West, East )

# subset coordinates and unrotate
sub_lon2 = modf.subset_1d(theta_lon, West, East)
sub_lat2 = modf.subset_1d(theta_lat, South, North)
...
if region2_on == True:
    for level in range(40):
        for time in range(48):
            counter += 1
            print(counter, 'model-level:', level + 1, 'time-step:', time + 1)
            fig = modf.make_horizontal_model_contourf_linear(sub_data_region2, sub_land_region2, sub_lon2, sub_lat2, sub_lons2, sub_lats2, sub_lon2, sub_lat2,
                                                                     data_label = r'$\theta$, [K]', domain = 'subset2', 
                                                                     flight = 301, savefig = True, res = '1p5km',
                                                                     variable_name = 'Potentintal temperature', level = level, time = time,
                                                                     variable_in_file_name = 'air_potential_temperature',
                                                                     figure_path_pdf = '../Figures/PDF/301/Model_levels/Theta',
...
if animation_on == True:
    for j in range(40):
        ...
        #Add colorbar
        cbar = plt.colorbar( a, ax=ax, orientation='vertical'); cbar.ax.set_xlabel(r'$\theta$ [K]')
        fargs = (sub_data_region2, sub_lon2, sub_lat2,sub_land_region2, level, 0.5, my_cmap, ax, 'Potential temperature')
        ani = animation.FuncAnimation(fig,anif.animate_pcolor,48,interval=5,blit=False, fargs = fargs)
        ani.save(f'{figure_path_mp4}/subset2/1p5km_subset2_potential_temperature_20180312-301_24hrs_ML{level+1}.mp4', writer=writer, dpi=300)