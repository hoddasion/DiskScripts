# -*- coding: utf-8 -*-
"""
Created on Thu Feb 17 10:58:35 2022
...
for res in resolutions:
    for leg in [1,4,7,13,15]:
        ...
        time = control_w_cube.coord('time')
        dates = time.units.num2date(time.points)
        
        #%% get scalar distance ticklist for x axis labelling
        tick_coors, tick_dists = workshop.dist_tick_generator(control_w_cube, 'grid_latitude', ticknumber = 6, decprecision = 2)
        ...
        #%% rotate db coords
        # first load 3d cube
        CUBE3D = iris.load_cube(f'D:/Project/Model_Data/{suite}/RA1M_0p5km_um_air_potential_temperature_24hrs_pi_306.nc', 'air_potential_temperature')
        ...
        if aggregate_3hour_plots:
            ...
            for chunk in range(int(24/3)):
                ...
                mean_wind_cube = control_wind_cube[ss*chunk:ss+chunk*ss].collapsed('time', iris.analysis.MEAN)
                mean_theta_cube = control_theta_cube[ss*chunk:ss+chunk*ss].collapsed('time', iris.analysis.MEAN)
                mean_q_cube = control_q_cube[ss*chunk:ss+chunk*ss].collapsed('time', iris.analysis.MEAN)
                mean_ws_cube = control_ws_cube[ss*chunk:ss+chunk*ss].collapsed('time', iris.analysis.MEAN)
                mean_sh_cube = control_sh_cube[ss*chunk:ss+chunk*ss].collapsed('time', iris.analysis.MEAN)
                mean_lh_cube = control_lh_cube[ss*chunk:ss+chunk*ss].collapsed('time', iris.analysis.MEAN)
                mean_w_cube = control_w_cube[ss*chunk:ss+chunk*ss].collapsed('time', iris.analysis.MEAN)
                ...
                fig.suptitle(f'CONTROL - leg {leg} - 3 hour mean {boundstart}-{boundend}')
                
                plt.tight_layout()
                if False:
                    
                    plt.savefig(f'D:/Project/Figures/PNG/306/CONTROL/{suite}/Vertical/averages/{res}/three_hour/leg{leg}/{config}_CONTROL_3houravr_vert_xsections_{boundstart}_306_leg{leg}.png')
                ...
                if db_start > runstart and db_start < runend:
                    ...
                    if False:
                        ...
                        plt.savefig(f'D:/Project/Figures/PNG/306/CONTROL/{suite}/Vertical/averages/{res}/three_hour/obs/{config}_CONTROL_3houravr_vert_xsections_{boundstart}_306_leg{leg}_with_obs.png')
                #plt.show()    
                plt.close()
