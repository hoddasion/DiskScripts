...
if plevel:
    ...
    for res in ['0p5km','1p5km','4p4km']:
        ...
        filename = f'RA1M_{res}_um'
        filenames = [f'{nc_path}{filename}{stream}1_flt{flight}.nc',f'{nc_path}{filename}{stream}2_flt{flight}.nc',
                     f'{nc_path}{filename}{stream}3_flt{flight}.nc',f'{nc_path}{filename}{stream}4_flt{flight}.nc']
        cubes = iris.load(filenames, varnames).concatenate()
        lm_cube = iris.load_cube(f'{nc_path}{filename}{stream}1_flt{flight}.nc', 'land_binary_mask')
        alt_cube = iris.load_cube(f'{nc_path}{filename}{stream}1_flt{flight}.nc', 'surface_altitude')
        cubes.append(lm_cube)
        cubes.append(alt_cube)
        print(cubes, '\n\n\n')
        
        iris.save(cubes, f'{nc_path}{filename}24hrs_pressurevars_{flight}.nc')
