# -*- coding: utf-8 -*-
"""
Created on Mon Jul  6 08:18:05 2020
...
# main data cube 
wcube = iris.load_cube(f'../../Model_Data/{suite}/nc/Control/upward_air_velocity/{Case}_upward_air_velocity_24hrs_{stream}_301.nc', 'upward_air_velocity')
wdata = wcube.data
...
#%%
print(wcube.coord('pressure'))
print(np.nanmax(wcube.data))
#%%
for i in range(16):
    timemeans = []
    for j in range(24):
...
if domain == 'peninsulas':
    ...
    # subset 4d data
    sub_w = foundry.modf.subset_4d(wdata, South, North, West, East)
    ...
    gph_levels = np.arange(0,50000,5)
    cscale_max = np.nanmax(sub_w)+0.1
    cscale_min = np.nanmin(sub_w)-0.1
    if cscale_max > - cscale_min:
        cscale_min = - cscale_max
    elif - cscale_min > cscale_max:
        cscale_max = - cscale_min
    counter = 0
    ...
    for level in level_idx:
            ...
            for time in range(9):
                ...
                try:
                    ...
                    fig0,ax0,norm0 = foundry.plot_hoz_xsec_mass(sub_w, lm, sub_lon, sub_lat, lons, lats, sub_lmlon, sub_lmlat, time, level,
                                                                     data_label = r'Windspeed, $ms^{-1}$', domain = domain, 
                                                                     flight = 301, savefig = True, res = Case, unrotated = True,
                                                                     variable_name = 'Vertical windspeed', time_norm = True,
                                                                     variable_in_file_name = 'upward_wind_velocity',colourmap = 'seismic',
                                                                     figure_path_pdf = f'../../Figures/PDF/301/u-bu807/P_levels/upward_air_velocity/{Case}/manual_cscale',
                                                                     figure_path_png = f'../../Figures/PNG/301/u-bu807/P_levels/upward_air_velocity/{Case}/manual_cscale',
                                                                     contour_type = 'contourf', sampling_rate = 'threehourly',
                                                                     msp_contour = False ,
                                                                     ...
                                                                     cscale_override = True, cscale_min = cscale_min, cscale_max = cscale_max, verstash = True,
                                                                     pressure_labels = pressure_labels, PDF = False, sym_cmap = True)
